...
#define OST_CLOCK_SOURCE MXC_TMR_IBRO_CLK // one shot
#define PWM_CLOCK_SOURCE MXC_TMR_ISO_CLK // PWM
#define CONT_CLOCK_SOURCE MXC_TMR_APB_CLK // continuous
...
void PWMTimer(void)
{
    ...
    // number of ticks for a period
    unsigned int periodTicks = MXC_TMR_GetPeriod(PWM_TIMER, PWM_CLOCK_SOURCE, 16, FREQ);
    // number of ticks when PWM is high (during duty cycle)
    ...
    */
    ...
    tmr.pres = TMR_PRES_16; // =16 might due to bit mask
    tmr.mode = TMR_MODE_PWM;
    tmr.bitMode = TMR_BIT_MODE_16A; // use timer as 16-bit
    tmr.clock = PWM_CLOCK_SOURCE;
    // period ticks is number of ticks in one period
    tmr.cmp_cnt = periodTicks; // compare register value in timer ticks
    tmr.pol = 1; // polarity
