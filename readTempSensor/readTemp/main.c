...
/***** Preprocessors *****/
#define MASTERSYNC 1 // master synchronous, CPU waits until operation is done [x]
#define MASTERASYNC 0 // master asynchronous, not blocking CPU
#define MASTERDMA 0 // DMA -> direct memory access, CPU sets up the transfer once
// DMA allows the SPI peripheral to directly interface with the memory without the involvement of CPU
...
/***** Definitions *****/
#define DATA_LEN 100 // Words (16 bits)
#define DATA_VALUE 0xA5A5 // This is for master mode only... (1 word)
#define VALUE 0xFFFF
...
#define SPI MXC_SPI0 // use the SPI0 peripheral
#define SPI_IRQ SPI0_IRQn
#endif

#define SS_IDX 1 // modify spi_pins.ss1
...
int main(void)
{
    ...
    for (i = 2; i < 17; i++) {
        ...
        // returns the actual clock frequency if success
        ...
        req.txCnt = 0; // number of bytes transmitted from txData
        req.rxCnt = 0; // numof of bytes stored in rxData
        req.completeCB = (spi_complete_cb_t)SPI_Callback; // Pointer to function called when transaction is complete
        SPI_FLAG = 1;
